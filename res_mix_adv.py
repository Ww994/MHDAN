...
import torch
from advloss import AdversarialLoss
...
class MFSAN(nn.Module):
    ...
    def __init__(self, num_classes=31):
        ...
        self.avgpool = nn.AvgPool2d(7, stride=1)
        self.advloss = AdversarialLoss(1.0, 1000, use_lambda_scheduler=True)

    def forward(self, data_src, data_tgt = 0, label_src = 0, mark = 1,ratio=0.4):
        mmd_loss = 0
        # add
        mmd_loss_m=0
        data_mix=ratio*data_src+(1-ratio)*data_tgt
        ...
        if self.training == True:
            if mark == 1:
                ...
                #add
                data_mix=self.sharedNet(data_mix)
                ...
                aloss = self.advloss(data_src, data_tgt_son1)

                # add
                data_mix_son1 = self.sonnet1(data_mix)
                data_mix_son1 = self.avgpool(data_mix_son1)
                data_mix_son1 = data_mix_son1.view(data_mix_son1.size(0), -1)
                mmd_loss_m += mmd.mmd(data_src, data_mix_son1)
                ...
                # add
                pred_mix_src = self.cls_fc_son1(data_mix_son1)
                # mix_loss = mixup_criterion_hard(pred_mix_src, label_src, pred_src, ratio)
                mix_cls_loss = F.nll_loss(F.log_softmax(pred_mix_src, dim=1), label_src)

                cls_loss = F.nll_loss(F.log_softmax(pred_src, dim=1), label_src)

                return cls_loss, mmd_loss, mmd_loss_m, l1_loss,mix_cls_loss,aloss
            ...
            if mark == 2:
                ...
                aloss = self.advloss(data_src, data_tgt_son2)

                # add
                data_mix_son2 = self.sonnet2(data_mix)
                data_mix_son2 = self.avgpool(data_mix_son2)
                data_mix_son2 = data_mix_son2.view(data_mix_son2.size(0), -1)
                mmd_loss_m += mmd.mmd(data_src, data_mix_son2)
                ...
                # add
                pred_mix_src = self.cls_fc_son2(data_mix_son2)
                # mix_loss = mixup_criterion_hard(pred_mix_src, label_src, pred_src, ratio)
                mix_cls_loss = F.nll_loss(F.log_softmax(pred_mix_src, dim=1), label_src)
